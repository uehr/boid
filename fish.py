...
# v2: avoidFishsMoveAmount
# v3: nearFishsAverageMoveAmount

class fish(baseLife.baseLife):
    ...
    def toCenterMoveAmount(self, CENTER_PULL_FACTOR=80):
        ...
        for bard in self.otherFishs:
            y,x = bard.point
            moveY += y
            moveX += x

        moveY /= len(self.otherFishs)
        moveX /= len(self.otherFishs)
    ...
    def nearFishsAverageMoveAmount(self):
        moveY = 0
        moveX = 0
    ...
    def next(self, otherLifes):
        ...
        self.otherFishs = list(filter(lambda life: life.type == "fish", otherLifes))
        selfY, selfX = self.point

        v1 = self.toCenterMoveAmount()
        v2 = self.avoidFishsMoveAmount()
        v3 = self.nearFishsAverageMoveAmount()
